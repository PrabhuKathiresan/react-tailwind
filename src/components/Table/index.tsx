...
export const Table: React.FC<TableProps> = ({ children, layout = 'auto', className = ''}) => (
  <table className={buildClassName('border-collapse table-fixed w-full divide-y divide-gray-200', TABLE_LAYOUT_CLASSES[layout], className)}>
    {children}
  </table>
)

export const TabelHead: React.FC<TableHeadProps> = ({ children, className = '', onSort }) => (
  <thead className={buildClassName('text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400', className)}>
...
export const TabelBody: React.FC<TableBodyProps> = ({ children, loading, colSize, rowSize = 5, className = '' }) => (
  <tbody className={buildClassName('text-sm text-gray-700 dark:text-white divide-y divide-gray-200 dark:divide-gray-700', className)}>
    {
      loading ? (
...
export const TableRow: React.FC<TableRowProps> = ({ children, hoverable = true, className, ...props }) => (
  <tr className={buildClassName('last:border-b border-gray-200 dark:border-gray-700 group h-full', className)} {...props}>
    {children}
  </tr>
)

export const TableCell: React.FC<TableCellProps> = ({ children, align = 'left', className = '', ...props }) => (
  <td
    ...
    {...props}
  >
    <div className="antialiased leading-normal">
      {children}
    </div>
  </td>
)

export const TableHeaderCell: React.FC<TableHeaderCellProps> = ({ children, align = 'left', className = '', ...props }) => (
  <th
    className={
      buildClassName(
    ...
    {...props}
  >
    <BodyText as="div" className="block antialiased leading-none">{children}</BodyText>
  </th>
)
