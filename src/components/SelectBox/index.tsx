...
const constructInputClass = (disabled: boolean) => buildClassName(
  'transition-all',
    ...
    <div className={buildClassName('group space-y-2', hasError && 'has-error', containerClass)}>
      {
        label && (
          <div className={buildClassName('flex items-center justify-between', labelWrapperClass)}>
            <Label className={labelClass} htmlFor={id} aria-required={required}>{label}</Label>
            {labelHint}
          </div>
        )
      }
                ...
                isArray(selected) && selected?.length > 0 && (
                  <>
                    {selected.map((option, idx) => (
                      <Badge key={getValue(option)} theme="secondary" removable={!disabled} onRemove={() => removeSelection(idx)}>
            ...
            <div className="relative">
              ...
              />
              {
                allowClear && !isEmpty(selected) && (
                  <Button size="xs" iconOnly theme="secondary" variant="plain" className="group absolute inset-y-0 right-8 h-6 top-2" onClick={() => handleOnChange(multiple ? [] : null)} disabled={disabled}>
              ...
              <>
                {
                  filteredOptions.map((option) => (
                    <ComboboxOption key={getValue(option)} disabled={(option as SelectOption).disabled} value={option} className="group flex items-center justify-between gap-2 rounded-lg py-1.5 px-3 select-none data-[focus]:bg-black/3 dark:data-[focus]:bg-white/10 data-disabled:opacity-50 cursor-pointer data-disabled:pointer-events-none">
                ...
                {
                  allowAdd ? (
                    <div role="button" onClick={() => onAdd(query)} className="group flex items-center gap-2 rounded-lg py-1.5 px-3 cursor-pointer text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
