...
export const RadioSwitch: React.FC<RadioSwitchProps> = ({ label, items, selected, onChange }) => {
  ...
  return (
    <RadioGroup
      ...
      onChange={onChange}
      aria-label={label}
      className="flex items-center bg-white rounded-full shadow-xs p-1 dark:bg-gray-800 gap-1 border border-gray-200 dark:border-gray-700"
    >
      {items.map((item) => (
        <Radio
          key={getValue(item)}
          ...
          >
            <div className="text-sm/6">
              ...
              {(item as any).description && (
                <div className="flex gap-2 text-white/50">{getDescription(item)}</div>
              )}
            </div>
          </div>
        </Radio>
